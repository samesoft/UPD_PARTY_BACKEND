...
exports.createDistrict = async (req, res) => {
  try {
    ...
    const { district, stateid, regionid } = req.body;

    if (!district || !stateid || !regionid) {
    ...
    const [result] = await sequelize.query(
      `
        ...
        VALUES (:district_id, :district, :regionid, :stateid)
        RETURNING *;
        `,
      {
        replacements: {
...
exports.getAllDistricts = async (req, res) => {
  try {
    const result = await sequelize.query("SELECT * FROM district_get_all()", {
      type: sequelize.QueryTypes.SELECT,
    });
...
exports.updateDistrict = async (req, res) => {
  try {
    ...
    if (!id || !district || !regionid) {
      return res
        .status(400)
        .json({ error: "Missing required fields: id, district, or regionid" });
    }
